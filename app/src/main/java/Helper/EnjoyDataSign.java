...
public class EnjoyDataSign {
	public static void main(String[] args) {
		String i = "{'merchantId':1}";
		String b = "{\"merchantId\":1}";
		System.out.println(getDataSign(i, "asdasd"));
		System.out.println(getDataSign(b, "asdasd"));
	 ...
	 * @return
	 */
	public static List<String> JsonObjSort(JSONObject json){
	...
	public static String getDataSign(String s, String signkey) {
		System.out.println(String.format("获取数据签名，信息：%s，Key：%s", s,signkey));
		JSONObject jsonObj = null;
		try {
			jsonObj = new JSONObject(s);
			// 获取并剔除sign
			...
			for (String key : keylist) {
				...
				/*valuess += (key + "=" + getJobjKeyValue(obj)+"&");*/
				valuess += (key + "=" + obj.toString()+"&");
			}
			
			valuess+= "key=" + signkey;
			// sha1加密
			String sha1 = hex_sha1(valuess);
			return sha1;
		} catch (Exception e) {
			e.printStackTrace();
			return "";
	...
	private static String hex_sha1(String s) {
		s = (s == null) ? "" : s;
		System.out.println(String.format("签名哈希值：%s", s));
		return binb2hex(core_sha1(str2binb(s),s.getBytes(Charset.forName("utf-8")).length * chrsz));
	}
	...
	private static String hex_hmac_sha1(String key, String data) {
	...
	private static int[] str2binb(String str) {
		str = (str == null) ? "" : str;
		byte[] strbuf=str.getBytes(Charset.forName("utf-8"));
		int[] tmp = new int[strbuf.length * chrsz];
		int mask = (1 << chrsz) - 1;

		for (int i = 0; i < strbuf.length * chrsz; i += chrsz) {
			tmp[i >> 5] |= ((int) (strbuf[i / chrsz]) & mask) << (24 - i % 32);
		}
